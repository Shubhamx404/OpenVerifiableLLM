...
def test_extract_text_from_xml_end_to_end_bz2(tmp_path, monkeypatch):
...
def test_extract_text_from_xml_end_to_end_xml(tmp_path, monkeypatch):

    xml_content = """<?xml version="1.0"?>
    <mediawiki>
      <page>
        <revision>
          <text>Hello [[World]] Uncompressed</text>
        </revision>
      </page>
    </mediawiki>
    """
    ...
    assert "Hello World Uncompressed" in processed_file.read_text()

def test_extract_text_from_xml_invalid_extension(tmp_path):
    input_file = tmp_path / "simplewiki-20260201-pages.txt"
    input_file.write_text("not an xml file")

    with pytest.raises(ValueError, match="Unsupported file extension: .txt. Expected .bz2 or .xml"):
        utils.extract_text_from_xml(input_file)
