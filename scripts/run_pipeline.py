...
def main():
    parser = argparse.ArgumentParser(description="End-to-End Open-Everything Verifiable LLM Data Pipeline")
    parser.add_argument("dump_path", type=str, help="Path to the Wikipedia XML dump (e.g., .xml or .xml.bz2)")
    ...
    cmd1 = [
        sys.executable, "-m", "openverifiablellm.utils",
        str(dump_path)
    ]
    run_command(cmd1, "Step 1: Data Preprocessing (XML to Clean Text)")
    ...
    run_command(cmd2, "Step 2: Train BPE Tokenizer")

    # Step 3: Tokenize Data
    cmd3 = [
        sys.executable, "-m", "openverifiablellm.tokenize_data",
        "--text_path", str(clean_txt_path),
        "--tokenizer_path", str(tokenizer_dir),
        "--output_path", str(tokens_bin_path)
    ]
    run_command(cmd3, "Step 3: Tokenize Dataset to Binary format")
    ...
    run_command(cmd4, "Step 4: Deterministic Training Loop")
